...
public class Sorts {
    ...
    public static void init() {
        String k = SPs.getString(SORT_KEY);
        if (TextUtils.isEmpty(k)) {
            SPs.put(SORT_KEY, BY_NAME);
        }

        String o = SPs.getString(SORT_ORDER);
        if (TextUtils.isEmpty(o)) {
            SPs.put(SORT_ORDER, DESCENDING);
        }
    }

    public static String getSortKey() {
        return SPs.getString(SORT_KEY);
    }

    public static String getSortOrder() {
        return SPs.getString(SORT_ORDER);
    }

    public static void setSortType(int type) {
        if (type == SORT_BY_NAME_ASC) {
            SPs.put(SORT_KEY, BY_NAME);
            SPs.put(SORT_ORDER, ASCENDING);
        } else if (type == SORT_BY_NAME_DESC) {
            SPs.put(SORT_KEY, BY_NAME);
            SPs.put(SORT_ORDER, DESCENDING);
        } else if (type == SORT_BY_MODIFIED_TIME_ASC) {
            SPs.put(SORT_KEY, BY_MODIFIED_TIME);
            SPs.put(SORT_ORDER, ASCENDING);
