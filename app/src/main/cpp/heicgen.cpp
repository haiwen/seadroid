 ...
 */
static heif_image *CreateHeifImageFromRGBA(const uint8_t *rgba, int width, int height, int stride) {
...
JNICALL
Java_com_seafile_seadroid2_HeicNative_genStillHeicSeq(JNIEnv *env, jobject,
...
JNICALL
Java_com_seafile_seadroid2_HeicNative_nativeGetLibVersion(JNIEnv *env, jclass) {
    const char *version = heif_get_version();
    return env->NewStringUTF(version);
}


// ==================== Google Motion Photo Debug Logs ====================
#define TAG_MP "MotionPhoto"
...
static heif_error memory_writer_write(heif_context *ctx, const void *data,
    ...
    LOGD_MP("[MemoryWriter] write: %zu bytes (buffer: %zu -> %zu)", size, oldSize, writer->data.size());
...
static bool WriteMpvdBox(FILE *file, const std::vector <uint8_t> &mp4Data) {
    ...
    LOGD_MP("[WriteMpvdBox] boxSize=%u (header=8 + data=%zu)", boxSize, mp4Data.size());

    // 写入 box size (big-endian)
    uint8_t sizeBytes[4] = {
            static_cast<uint8_t>((boxSize >> 24) & 0xFF),
    ...
    if (mp4Data.size() >= 12) {
        LOGD_MP("[WriteMpvdBox] MP4 header bytes: [%02X %02X %02X %02X] [%c%c%c%c] [%c%c%c%c]",
                mp4Data[0], mp4Data[1], mp4Data[2], mp4Data[3],
                mp4Data[4], mp4Data[5], mp4Data[6], mp4Data[7],
                mp4Data[8], mp4Data[9], mp4Data[10], mp4Data[11]);
    }

    // 写入 MP4 视频数据
    size_t written = fwrite(mp4Data.data(), 1, mp4Data.size(), file);
    if (written != mp4Data.size()) {
        LOGE_MP("[WriteMpvdBox] FAILED to write MP4 data (wrote %zu of %zu)", written, mp4Data.size());
        return false;
    }
 ...
 */
static heif_image_handle *EncodePrimaryImageForMotionPhoto(const std::vector <uint8_t> &jpegBytes,
                                                           heif_context *ctx) {
    ...
    LOGI_MP("[EncodePrimary] JPEG decoded: %dx%d, components=%d", width, height, comps);
    ...
    size_t rgba_stride = static_cast<size_t>(width) * 4;
    std::vector <uint8_t> rgba(static_cast<size_t>(height) * rgba_stride);
    LOGD_MP("[EncodePrimary] Allocated RGBA buffer: %zu bytes (stride=%zu)", rgba.size(), rgba_stride);
    ...
    LOGI_MP("[EncodePrimary] SUCCESS - handle=%p, image=%dx%d", handle, width, height);
    return handle;
}
 ...
 *   - ftyp box
 *   - meta box
 *      - XMP 里须包含 Google Motion Photo 标识字段：例如(G)Camera:MotionPhoto、(G)Camera:MotionPhotoVersion、(G)Camera:MotionPhotoPresentationTimestampUs
...
JNICALL
Java_com_seafile_seadroid2_HeicNative_nativeGenGoogleMotionPhotoWithHeic(JNIEnv *env, jobject,
    ...
    LOGI_MP("[Step 3/6] Primary image encoded successfully, handle=%p", primaryHandle);
    ...
    heif_error xmpErr = heif_context_add_XMP_metadata(ctx, primaryHandle,
                                                      xmpData.data(), static_cast<int>(xmpData.size()));
    ...
    LOGI_MP("[Step 5/6] HEIC written to memory: %zu bytes", memWriter.data.size());

    // 打印 HEIC 文件开头
    if (memWriter.data.size() >= 16) {
        LOGD_MP("[Step 5/6] HEIC header: [%02X %02X %02X %02X] [%c%c%c%c] [%c%c%c%c]",
                memWriter.data[0], memWriter.data[1], memWriter.data[2], memWriter.data[3],
                memWriter.data[4], memWriter.data[5], memWriter.data[6], memWriter.data[7],
                memWriter.data[8], memWriter.data[9], memWriter.data[10], memWriter.data[11]);
    }
    ...
    if (!outFile) {
        LOGE_MP("[Step 6/6] FAILED to open output file: %s (errno=%d)", outPath, errno);
        env->ReleaseStringUTFChars(outputPath, outPath);
    ...
    // 写入 HEIC 数据
    size_t heicWritten = fwrite(memWriter.data.data(), 1, memWriter.data.size(), outFile);
    if (heicWritten != memWriter.data.size()) {
        LOGE_MP("[Step 6/6] FAILED to write HEIC data (wrote %zu of %zu)", heicWritten, memWriter.data.size());
        fclose(outFile);
        env->ReleaseStringUTFChars(outputPath, outPath);
    ...
    LOGI_MP("  mpvd box:    %zu bytes (offset %zu)", 8 + mp4Data.size(), memWriter.data.size());
    LOGI_MP("    - header:  8 bytes");
    LOGI_MP("    - MP4:     %zu bytes", mp4Data.size());
    LOGI_MP("  Total size:  %zu bytes (%.2f MB)", totalSize, totalSize / (1024.0 * 1024.0));
    LOGI_MP("============================================================");
    ...
    snprintf(result, sizeof(result),
             ...
             memWriter.data.size(), mp4Data.size(), totalSize, totalSize / (1024.0 * 1024.0));
    ...
    if (numMetadata > 0) {
        ...
        for (int i = 0; i < numMetadata; i++) {
            ...
            bool isXmp = (contentType && strcmp(contentType, "application/rdf+xml") == 0) ||
                         (type && strcmp(type, "mime") == 0 && contentType && strstr(contentType, "xmp") != nullptr);
            ...
            if (isXmp) {
                ...
                if (metadataSize > 0) {
                    ...
                    if (err.code == heif_error_Ok) {
                        info.xmpContent = std::string(reinterpret_cast<const char *>(xmpData.data()), metadataSize);
                        LOGD_MP("[ParseHeicXMP] XMP content loaded, size=%zu", info.xmpContent.size());

                        // 解析 GCamera:MotionPhoto
                        std::string motionPhoto = ExtractXmpValue(info.xmpContent, "GCamera:MotionPhoto");
                        if (motionPhoto == "1") {
                            info.isMotionPhoto = true;
                            LOGD_MP("[ParseHeicXMP] GCamera:MotionPhoto = 1 (confirmed Motion Photo)");
                        }
                        ...
                        if (!version.empty()) {
                            ...
                            LOGD_MP("[ParseHeicXMP] GCamera:MotionPhotoVersion = %d", info.version);
                        }
                        ...
                        if (motionPhotoItemPos != std::string::npos) {
                            ...
                            if (!lengthStr.empty()) {
                                ...
                                LOGD_MP("[ParseHeicXMP] MotionPhoto Item:Length = %ld", info.videoLength);
                            }

                            std::string paddingStr = ExtractXmpValue(itemContext, "Item:Padding");
                            if (!paddingStr.empty()) {
                                info.videoPadding = std::stol(paddingStr);
                                LOGD_MP("[ParseHeicXMP] MotionPhoto Item:Padding = %ld", info.videoPadding);
                            }

                            info.videoMime = ExtractXmpValue(itemContext, "Item:Mime");
                            if (!info.videoMime.empty()) {
                                LOGD_MP("[ParseHeicXMP] MotionPhoto Item:Mime = %s", info.videoMime.c_str());
                            }
                        }
...
JNICALL
Java_com_seafile_seadroid2_HeicNative_nativeExtractGoogleHeicMotionPhotoVideo(JNIEnv *env, jobject,
    ...
    if (!file) {
        LOGE_MP("[ExtractHEIC] Failed to open file: %s (errno=%d)", filePath, errno);
    ...
    LOGD_MP("[ExtractHEIC] File size: %ld bytes (%.2f MB)", fileSize, fileSize / (1024.0 * 1024.0));
    ...
    if (bytesRead != static_cast<size_t>(fileSize)) {
        LOGE_MP("[ExtractHEIC] Failed to read file (read %zu of %ld)", bytesRead, fileSize);
        env->ReleaseStringUTFChars(inputFilePath, filePath);
        return nullptr;
    }
    ...
    if (xmpInfo.isMotionPhoto && xmpInfo.videoLength > 0) {
        LOGI_MP("[ExtractHEIC] XMP method: Motion Photo confirmed, video length = %ld", xmpInfo.videoLength);
        ...
        long padding = xmpInfo.videoPadding > 0 ? xmpInfo.videoPadding : 8;  // 默认 8 字节 mpvd 头
        ...
        if (mp4StartPos > 0 && mp4StartPos < fileSize) {
            ...
            if (mp4StartPos + 8 <= fileSize) {
                if (fileData[mp4StartPos + 4] == 'f' &&
                    ...
                    LOGD_MP("[ExtractHEIC] XMP method: Verified ftyp at calculated position (padding=%ld)", padding);
                } else {
                    LOGW_MP("[ExtractHEIC] XMP method: No ftyp at position %ld, trying with padding adjustment", mp4StartPos);
                    // 尝试使用 padding 调整
                    ...
                    if (adjustedPos > 0 && adjustedPos + 8 <= fileSize &&
                        ...
                        LOGD_MP("[ExtractHEIC] XMP method: Found ftyp at adjusted position %ld", adjustedPos);
                    } else {
                        // XMP 方式失败，让回退到 mpvd 搜索
                        LOGW_MP("[ExtractHEIC] XMP method: Cannot locate video, will try mpvd search");
                        mp4StartPos = -1;
                    }
                }
            }
            ...
            if (mp4StartPos > 0) {
                LOGD_MP("[ExtractHEIC] XMP method: MP4 starts at offset %ld, size %u", mp4StartPos, mp4Size);
            }
        } else {
            LOGW_MP("[ExtractHEIC] XMP method: Invalid calculated position %ld", mp4StartPos);
            mp4StartPos = -1;
        }
    } else {
        if (!xmpInfo.isMotionPhoto) {
            LOGD_MP("[ExtractHEIC] XMP method: Not a Google Motion Photo (GCamera:MotionPhoto != 1)");
        } else {
            LOGD_MP("[ExtractHEIC] XMP method: No video length in XMP (Item:Length not found)");
        }
    }
    ...
    if (mp4StartPos < 0) {
        ...
        for (long i = fileSize - 4; i >= 4; --i) {
            if (fileData[i] == mpvdSignature[0] &&
                ...
                mpvdPos = i;
                LOGD_MP("[ExtractHEIC] mpvd method: Found 'mpvd' signature at offset %ld", mpvdPos);
                break;
            }
        }
        ...
        if (mpvdPos >= 4) {
            ...
            if (boxSize >= 8) {
                ...
                LOGD_MP("[ExtractHEIC] mpvd method: MP4 starts at offset %ld, size %u", mp4StartPos, mp4Size);
            } else {
    ...
    if (mp4StartPos + mp4Size > static_cast<unsigned long>(fileSize)) {
        LOGE_MP("[ExtractHEIC] MP4 data exceeds file bounds (offset=%ld, size=%u, fileSize=%ld)",
    ...
    if (mp4Size >= 12 && mp4StartPos + 12 <= fileSize) {
        uint32_t firstBoxSize = (static_cast<uint32_t>(fileData[mp4StartPos]) << 24) |
        ...
        LOGD_MP("[ExtractHEIC] MP4 first box: size=%u, type='%c%c%c%c', brand='%c%c%c%c'",
                firstBoxSize,
    ...
    if (!result) {
        LOGE_MP("[ExtractHEIC] Failed to allocate Java byte array for %u bytes", mp4Size);
        env->ReleaseStringUTFChars(inputFilePath, filePath);
        return nullptr;
    }
    ...
    if (xmpInfo.isMotionPhoto) {
        LOGI_MP("  XMP info: MotionPhoto=1, version=%d, videoLength=%ld, padding=%ld",
                xmpInfo.version, xmpInfo.videoLength, xmpInfo.videoPadding);
    }
    LOGI_MP("  MP4 offset: %ld", mp4StartPos);
    LOGI_MP("  MP4 extracted: %u bytes (%.2f MB)", mp4Size, mp4Size / (1024.0 * 1024.0));
    LOGI_MP("============================================================");
    ...
    while (pos < fileData.size() - 4) {
        ...
        if (marker == 0xE1) {
            ...
            if (segmentDataLen > XMP_MARKER_LEN + 1 &&
                memcmp(fileData.data() + segmentDataStart, XMP_MARKER, XMP_MARKER_LEN) == 0) {
                ...
                if (xmpStart + xmpLen <= fileData.size()) {
                    info.xmpContent = std::string(reinterpret_cast<const char *>(fileData.data() + xmpStart), xmpLen);

                    LOGD_MP("[ParseXMP] Found XMP segment at offset %zu, length %zu", pos, xmpLen);
                    ...
                    // 查找 MotionPhoto Item 的 Length
                    // 结构: <rdf:li ...><Item:Semantic>MotionPhoto</Item:Semantic><Item:Length>xxx</Item:Length>...</rdf:li>
                    size_t motionPhotoItemPos = info.xmpContent.find("MotionPhoto</Item:Semantic>");
                    if (motionPhotoItemPos == std::string::npos) {
                        // 也尝试查找属性格式
                        motionPhotoItemPos = info.xmpContent.find("Item:Semantic=\"MotionPhoto\"");
...
JNICALL
Java_com_seafile_seadroid2_HeicNative_nativeExtractGoogleJpegMotionPhotoVideo(JNIEnv *env, jobject,
                                                                             jstring inputFilePath) {
    LOGI_MP("============================================================");
    ...
    FILE *file = fopen(filePath, "rb");
    if (!file) {
        LOGE_MP("[ExtractJPEG] Failed to open file: %s (errno=%d)", filePath, errno);
        env->ReleaseStringUTFChars(inputFilePath, filePath);
        return nullptr;
    }
    ...
    long fileSize = ftell(file);
    fseek(file, 0, SEEK_SET);
    LOGD_MP("[ExtractJPEG] File size: %ld bytes (%.2f MB)", fileSize, fileSize / (1024.0 * 1024.0));
    ...
    if (bytesRead != static_cast<size_t>(fileSize)) {
        LOGE_MP("[ExtractJPEG] Failed to read file (read %zu of %ld)", bytesRead, fileSize);
        env->ReleaseStringUTFChars(inputFilePath, filePath);
        return nullptr;
    }
    ...
    if (xmpInfo.isMotionPhoto && xmpInfo.videoLength > 0) {
        LOGI_MP("[ExtractJPEG] XMP method: Motion Photo confirmed, video length = %ld", xmpInfo.videoLength);
        ...
        if (mp4StartPos > 0 && mp4StartPos < fileSize) {
            ...
            if (mp4StartPos + 8 <= fileSize) {
                ...
                if (fileData[mp4StartPos + 4] == 'f' &&
                    ...
                    fileData[mp4StartPos + 6] == 'y' &&
                    fileData[mp4StartPos + 7] == 'p') {
                    LOGD_MP("[ExtractJPEG] XMP method: Verified ftyp at calculated position");
                } else {
                    LOGW_MP("[ExtractJPEG] XMP method: No ftyp at calculated position, data may be invalid");
            ...
            LOGD_MP("[ExtractJPEG] XMP method: MP4 starts at offset %ld", mp4StartPos);
        } else {
            LOGW_MP("[ExtractJPEG] XMP method: Invalid calculated position %ld", mp4StartPos);
            mp4StartPos = -1;
        }
    } else {
        if (!xmpInfo.isMotionPhoto) {
            LOGD_MP("[ExtractJPEG] XMP method: Not a Google Motion Photo (GCamera:MotionPhoto != 1)");
        } else {
            LOGD_MP("[ExtractJPEG] XMP method: No video length in XMP (Item:Length not found)");
        }
    }

    // ==================== 方式二：回退到 ftyp 搜索方式 ====================
    if (mp4StartPos < 0) {
        LOGI_MP("[ExtractJPEG] Falling back to ftyp search method (Huawei style)...");
        ...
        for (long i = searchStart; i < fileSize - 4; ++i) {
            if (fileData[i] == ftypSignature[0] &&
                ...
                ftypPos = i;
                LOGD_MP("[ExtractJPEG] ftyp method: Found 'ftyp' signature at offset %ld", ftypPos);
                break;
            }
        }
        ...
        if (ftypPos >= 4) {
            ...
            uint32_t ftypBoxSize = (static_cast<uint32_t>(fileData[mp4StartPos]) << 24) |
                                   (static_cast<uint32_t>(fileData[mp4StartPos + 1]) << 16) |
                                   (static_cast<uint32_t>(fileData[mp4StartPos + 2]) << 8) |
                                   static_cast<uint32_t>(fileData[mp4StartPos + 3]);

            LOGD_MP("[ExtractJPEG] ftyp method: ftyp box size = %u bytes", ftypBoxSize);

            // 验证 ftyp box size 是否合理
            if (ftypBoxSize < 8 || ftypBoxSize > 256) {
                LOGW_MP("[ExtractJPEG] ftyp method: Unusual ftyp box size: %u (expected 8-256)", ftypBoxSize);
            }

            LOGD_MP("[ExtractJPEG] ftyp method: MP4 starts at offset %ld", mp4StartPos);
        } else {
            LOGE_MP("[ExtractJPEG] ftyp method: 'ftyp' not found or invalid position");
        }
    }
    ...
    if (mp4Size >= 12 && mp4StartPos + 12 <= fileSize) {
        ...
        LOGD_MP("[ExtractJPEG] MP4 first box: size=%u, type='%c%c%c%c', brand='%c%c%c%c'",
                firstBoxSize,
                fileData[mp4StartPos + 4], fileData[mp4StartPos + 5],
                fileData[mp4StartPos + 6], fileData[mp4StartPos + 7],
                fileData[mp4StartPos + 8], fileData[mp4StartPos + 9],
    ...
    LOGD_MP("[ExtractJPEG] JPEG image size: %ld bytes (%.2f KB)", jpegSize, jpegSize / 1024.0);

    // 创建 Java byte array
    jbyteArray result = env->NewByteArray(static_cast<jsize>(mp4Size));
    if (!result) {
        LOGE_MP("[ExtractJPEG] Failed to allocate Java byte array for %u bytes", mp4Size);
        env->ReleaseStringUTFChars(inputFilePath, filePath);
        return nullptr;
    }
