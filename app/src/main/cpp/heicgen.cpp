 ...
 */
static MotionPhotoXmpInfo
ParseJpegMotionPhotoXmp(const std::vector<uint8_t> &fileData) {
    MotionPhotoXmpInfo info;
...
Java_com_seafile_seadroid2_jni_HeicNative_nativeConvertJpegMotionPhotoToHeic(
        JNIEnv *env, jclass clazz, jstring inputJpegFilePath,
...
Java_com_seafile_seadroid2_jni_HeicNative_nativeExtractGoogleHeicMotionPhotoVideo(
    ...
    // ==================== 方式一：mpvd box 搜索（优先） ====================
    LOGI_MP("[ExtractHEIC] Trying mpvd box search method...");
    ...
    for (long i = fileSize - 4; i >= 4; --i) {
        if (fileData[i] == mpvdSignature[0] &&
            ...
            mpvdPos = i;
            LOGD_MP("[ExtractHEIC] mpvd method: Found 'mpvd' signature at offset %ld",
                    mpvdPos);
            break;
        }
    }
    ...
    if (mpvdPos >= 4) {
        ...
        LOGW_MP("[ExtractHEIC] mpvd method: 'mpvd' box not found");
    }

    // ==================== 方式二：回退到 XMP (libheif) ====================
    if (mp4StartPos < 0) {
        LOGI_MP("[ExtractHEIC] Falling back to XMP method using libheif API...");
        xmpInfo = ParseHeicMotionPhotoXmpWithLibheif(filePath);

        if (xmpInfo.isMotionPhoto && xmpInfo.videoLength > 0) {
            LOGI_MP("[ExtractHEIC] XMP method: Motion Photo confirmed, video length = %ld",
                    xmpInfo.videoLength);
                ...
                if (mp4StartPos + 8 <= fileSize) {
                    if (fileData[mp4StartPos + 4] == 'f' &&
                        ...
                        if (adjustedPos > 0 && adjustedPos + 8 <= fileSize &&
                            ...
                            LOGD_MP(
                                    "[ExtractHEIC] XMP method: Found ftyp at adjusted position (minus padding) %ld",
                                    adjustedPos);
                        } else {
                            // 尝试 Case 3: Item:Length 包含了 Padding (例如 Xiaomi 格式)
                            // 视频起始位置 = fileSize - (videoLength - padding)
                            ...
                            if (adjustedPos2 > 0 && adjustedPos2 + 8 <= fileSize &&
                                ...
                                LOGD_MP(
                                        "[ExtractHEIC] XMP method: Found ftyp at adjusted position (length includes padding) %ld",
                                        adjustedPos2);
                            } else {
                                // XMP 方式失败，让回退到 mpvd 搜索
                                LOGW_MP("[ExtractHEIC] XMP method: Cannot locate video");
                                mp4StartPos = -1;
                            }
                        }
                    }
...
/**
 * 需求：提取 JPEG 格式的 Google Motion Photo 动态照片中的视频数据
 *
 * 逻辑：
 * 方式 1：通过读取图片的 XMP 数据，获取 Motion Photo 的视频偏移量
 * 方式 2：通过检索 “ftyp” 字符，直接定位 mp4 box 的偏移量
 * */
JNICALL
